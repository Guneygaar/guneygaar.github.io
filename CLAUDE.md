...
Last updated: 2026-05-03 (#1108 — Plan not mounting on fresh OTP sign-in. Root cause: `window.AppState.user` is mutated in place by `activateRole()` in `03-auth.js` (e.g. `window.AppState.user.role = role`); `syncFromWindow` in `srtd-next/src/core/stores/appState.ts` passed that same reference into Zustand's `set()`, and Zustand's `Object.is` change detection short-circuited the update — so `App.jsx` never re-rendered after `sorted:role-ready` and Plan never mounted on a fresh OTP login. Session resume worked because tokens were already in localStorage at the first `App.jsx` render, making `isAuthed()` true on the initial mount with no re-render needed. **Fix (A) — `appState.ts syncFromWindow` spreads user/workspace** so every sync produces new object identity; Zustand fires `set()`, `useUser()` re-emits, and `App.jsx`'s `authed = !!user?.role || isAuthed()` gate flips, mounting Plan. **Fix (B) — `03-auth.js` activateRole tail switchTab calls deleted** (the four lines that called `switchTab('tasks')` for both client and non-client branches). Plan owns the home surface at z-index 1400 once Fix A lands; activating dashboard-view's `tasks` panel underneath was dead weight. `loadTasks()` still runs earlier in the same function. **Fix (C) — `10-ui.js switchTab` dashboard-view activation now guards on `!document.body.classList.contains('plan-active')`** so future callers (`closeInsights`, `closeLibrary`, etc.) cannot re-paint dashboard-view under the Plan overlay. **Audit (no changes)**: other `window.AppState.user` writers — `12-profiles.js enrichAppStateUser` / `saveProfile` / avatar upload (mutates `displayName`/`avatarUrl`/`title`/`username`), `04-router.js` (sets `name`/`email`/`effectiveRole` BEFORE `activateRole` runs and is therefore covered by `_dispatchRoleReady`), `02-session.js` (init-time, before React mounts) — none are auth-state mutations and none gate Plan mounting. Out of scope. **New test**: `tests/appstate-sync.test.js` (5 cases) imports the .ts store and asserts (1) syncFromWindow produces a NEW user reference after each sync, (2) same for workspace, (3) Zustand `subscribe()` listener fires after an in-place mutation + sync (the regression check), (4) `hydrated` flips and stays true, (5) null window.AppState.user yields null in the store. Bundle rebuilt: `dist/sorted-react.js` 686.88 kB / `dist/style.css` 39.30 kB. 28 `?v=` strings bumped `20260502j/k/l -> 20260502m` (converged after prior partial bumps). Vitest: 584/584 passing. `npx tsc --noEmit` clean.
Last updated: 2026-05-02 (Rung 1 — stop briefs vanishing on Plan board across naked /posts merges. Root cause: `mergePosts` (07-post-load.js:138-148) does a complete-replace merge — anything not in the `fresh` array is deleted from `AppState.posts.all`. Briefs are synthesized from `/requests` and folded into `posts.all` ONLY by the full `loadPosts()` path (07-post-load.js:240). Four other call sites pass `/posts`-only data into `mergePosts` (10s realtime-poll fallback ~L498, `_drainPollStash` ~L533 fired on every modal close, agency realtime stash branch ~L879, task-completion sync ~L1099) — each one stripped every `_isRequest:true` brief, briefs reappeared on the next full `loadPosts()`, then vanished on the next naked merge. **Fix (A) — `mergePosts` deletion guard**: replaced `map.forEach((_, id) => { if (!freshIds.has(id)) map.delete(id); })` with a callback form that early-returns on `p && p._isRequest` so synthesized briefs survive `/posts`-only merges. Comment block in code documents WHY (briefs come from a parallel `/requests` fetch). Behavior change is "preserve a class of entries during merge" — exercised by every existing brief-load flow, no new tests needed; `mergePosts` signature, sort order, and `sorted:posts-updated` event dispatch unchanged. **Fix (B) — `/requests` fetches wrapped in `withRetry`**: 07-post-load.js:210 (agency `loadPosts`) and 07-post-load.js:319 (`loadPostsForClient`) were the only `apiFetch` calls in the load path NOT wrapped in `withRetry` while the parallel `/posts` fetch on lines 205 and 309 was. A transient blip dropped briefs even on the healthy load path. Both flipped to `await withRetry(function() { return apiFetch(...); })`. **Out of scope (later rungs)**: `/requests` → `/briefs` rename, `_isRequest` flag rename, brief lifecycle refactor. **Vanilla-only fix** — no React bundle, edge function, SQL, or worker changes. 28 `?v=` strings bumped `20260502a -> 20260502b`. Vitest: 552/552 passing.
## 1 — WHAT IS SORTED
...
5. Every push response must include: files changed, test count, version bump, PR URL.
...
## MULTI-FIX PR RULE — NON-NEGOTIABLE
When a single PR contains multiple distinct changes, each change gets its own one-paragraph justification tied to a specific symptom or test. If a fix's justification depends on another fix in the same PR being correct, the fixes are not independent — collapse them into one fix or split into sequential PRs. Re-validate every fix's justification any time the diagnosis changes mid-PR; a fix written under an old frame is dead weight under a new one. PR #1108 shipped Fix B (deleting `switchTab('tasks')` from `activateRole` agency tail) bundled with Fix A (the actual `appState.ts` reference-equality fix) — Fix B's premise ("Plan owns the home surface") was true on `?plan_react=1` but false on `?plan_react=0`, broke 8 PCS E2E tests, and required a patch in #1109 to gate on `plan_react=0`. Catching this at scope-time would have saved a round trip.
...
3. Architecture changes update the relevant section IN PLACE. They do not append a new `Last updated` block. Schema column added → update §2. New invariant → update §4. New flag → update §9. A `Last updated` entry is only justified for cross-cutting changes that don't fit any single section.

4. Any PR that adds more than 30 lines to CLAUDE.md must explain why in the PR description. Default assumption: if a change needs >30 lines to document, the change is too big — not the doc.

5. Forbidden content in CLAUDE.md: audit logs ("we tried X, then Y, then Z"), commit-by-commit history, test counts (drift), bundle sizes (drift), `?v=` values (drift), PR numbers used as section anchors. PR numbers may appear sparingly in body text where they pin a specific lesson (e.g. `## MULTI-FIX PR RULE` references #1108) but never as the structural identifier of a section.

6. Architecture changes to CLAUDE.md ship in the SAME PR as the code change. PR-history notes do not go in CLAUDE.md at all — use the PR description for those.
