...
Last updated: 2026-04-16 (fix-mention-display: five fixes for @mention resolution + comment author display. (1) Client feed card preview now calls `getDisplayName(latestAgency.author)` (render/client.js:954) and reply indicator calls `getDisplayName(author)` (render/client.js:1174) — previously both displayed raw email addresses like "thakur.manisha@somaiya.com" instead of display names. The main comment bubble at line 645 already used getDisplayName correctly. (2) PCS roster (`_fetchPcsRoster`, actions/pcs.js:3157) hardened with three-tier name resolution: `r.name.trim()` → `getDisplayName(r.email)` from profiles cache → email local-part split on `[._-]` with Title Case join. Previously `r.name || r.email.split('@')[0]` produced dot-containing names like "thakur.manisha" when user_roles.name was empty string. Roster cache now expires after 5 minutes (`_pcsRoster._ts` TTL check) instead of lasting the entire session, so user_roles/profiles updates are picked up on next PCS open. Roster exposed as `window._pcsRosterData` for cross-module access. (3) PCS mention extraction (`submitPcsComment`, actions/pcs.js:2938) replaced blind regex `/@([a-zA-Z0-9_]+)/g` with roster-aware matching: iterates `_pcsRoster` entries and checks `message.toLowerCase().indexOf('@' + name.toLowerCase())`, preserving full display names like "Manisha" in `mentioned_users`. Regex retained as fallback for manually-typed mentions not from the picker. Root cause of the `mentioned_users: ["thakur","somaiya"]` fragments seen in post_comments April 15 — the regex stopped at dots in "thakur.manisha". (4) `getDisplayName` email fallback (12-profiles.js:84) changed from `local.charAt(0).toUpperCase() + local.slice(1)` (produced "Shubham.gune") to `local.split(/[._-]/).filter(Boolean).map(Title Case).join(' ')` (produces "Shubham Gune"). Matches the existing `_displayNameForRow` pattern in render/client.js:1576. (5) Notification panel reply mention parsing (10-ui.js:1075) converted to roster-aware matching using `window._clientMentionRoster || window._pcsRosterData`, with the old regex as fallback. Zero schema changes.) Full history: `CLAUDE-archive-20260411.md`.
