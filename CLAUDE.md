...
Last updated: 2026-04-12 (notif-thread-polish). Full history: `CLAUDE-archive-20260411.md`.
...
- **Expand-in-place thread view** — `_buildItem` computes `postCommentCount = _notifPostCommentCount(n.post_id)` (non-resolved comments on the post from `window._notifComments`) and emits a gold `.expand-chip` whenever the count > 1. The chip label shows `postCommentCount` — not the notification grouping count — so any post with multiple comments gets a chip regardless of how the notifications were bucketed. Tap on the card body OR the chip toggles `.notif-item.expanded`, mounts the thread HTML, anchors scroll, and marks the notif read on first expand. State persists across `scroll.innerHTML` rebuilds via `window._notifExpandedSet` (Set of notif IDs). Thread HTML is cached per post in `window._notifThreadCache` (cleared at the top of `loadNotifications`).
...
- Drawer visuals: `.thread-area` carries `background: #1E1A2A` + `border: 1px solid #C8A84B26` + `border-radius: 10px` + `padding: 12px` + `margin-top: 8px` so it's clearly distinct from the panel background. `.notif-item.expanded` uses `background: #16161F` + `border-left: 3px solid #C8A84B4D` + `padding-bottom: 14px` + `margin-bottom: 6px` so the expanded card pops off its siblings. `.notif-item.expanded .thread-drawer` uses `max-height: 3000px` so the reply row + "Open full post →" link never clip. `.thread-reply` uses `border-top: 1px solid #C8A84B26` as the reply divider; `.thread-msg` uses `padding: 6px 0` + `border-bottom: 1px solid #FFFFFF0F` for tight message separators. `.thread-av` is 22×22 (vs 32px on notification cards), `.thread-message` is 12px (vs 13px).
- The drawer contains a `.reply-input` + `.reply-send` row and an `Open full post →` footer link. Replies POST to `/post_comments` ONLY — the JS fan-out is intentionally skipped so `notify-comment` (edge) is the single writer and there are no duplicate notification rows. Reply author is always `AppState.user.name`, author_role is `effectiveRole` title-cased, so the same code path works for both agency and client users. The panel tap delegate skips `.expand-chip`, `.reply-input`, `.reply-send`, `.thread-msg`, `.thread-area`, `.thread-reply`, `.thread-footer`, `.reply-label`, and routes `.thread-open-post` / `.thread-overflow` clicks through `closeNotifications()` + `openPCS` / `_openClientPostOverlay`.
...
1. Bump ALL 21 `?v=YYYYMMDDx` strings in `index.html` together (1 stylesheet + 20 scripts). Current: `?v=20260412c`.
