...
Last updated: 2026-04-13 (agency-realtime-subscriptions + client-realtime-subscriptions + refresh-400-recovery + collapse-timers + interval-rollback + avatar-crop-modal + avatar-cache-buster). Full history: `CLAUDE-archive-20260411.md`.
...
- `12-profiles.js` — Profile data loader and helpers. Fetches `profiles` table on login, caches in `window._profilesCache` (keyed by email). Also fetches `user_roles` to build `window._nameToRoleCache` (maps names and emails to roles) and `window._nameToEmailCache` (maps short names to emails for avatar/display-name resolution). Exposes: `getDisplayName(email)`, `getAvatarUrl(email)`, `getProfileByEmail(email)`, `getRoleFor(nameOrEmail)`, `updateLastActive()`, `fetchProfiles()`, `enrichAppStateUser()`, `renderAvatar(emailOrName, role, size, opts)`, `_avatarColors(role)`, `openProfilePanel()`, `closeProfilePanel()`, `saveProfile()`, `handleAvatarUpload(file)`, `_renderProfileTrigger()`. All helpers return graceful fallbacks if cache not loaded or fetch failed. `enrichAppStateUser()` sets `AppState.user.displayName`, `.avatarUrl`, `.title`, `.username` from the cached profile after fetch, then calls `_renderProfileTrigger()` to populate the topbar avatar. `getRoleFor()` resolves any name or email to a canonical role string via `_nameToRoleCache`; returns `''` for unknown inputs. Profile panel: full-screen overlay (`#prof-overlay`, z-index 1300) for editing own display name, username, title, notification prefs, and uploading avatar photo. Opens via `openProfilePanel()` (topbar avatar trigger), closes via `closeProfilePanel()`. `saveProfile()` PATCHes `/profiles` and updates local caches + AppState. **Avatar crop modal** (`#prof-crop-overlay`, z-index 1400) sits between file selection and `handleAvatarUpload`: file picker `onchange` calls `openAvatarCropModal(file)` which validates type (jpeg/png/webp) + size (<10MB raw), reads via FileReader/Image, then mounts a stage canvas (mobile-first 240–340px) under an SVG circular mask + gold ring overlay (`#C8A84B`). State (`window._cropState = { img, originalFile, stage, radius, scale, minScale, maxScale, tx, ty }`) is mutated by `_cropTouchStart`/`_cropTouchMove`/`_cropTouchEnd` (passive:false for iOS Safari pinch-zoom), `_cropMouseDown` (drag-pan), `_cropWheel` (8% per tick), and a zoom range slider. `_cropConstrain` clamps scale to `[minScale, maxScale]` and tx/ty so the image always fully covers the circle. `_cropDraw` uses `devicePixelRatio` for retina-crisp painting. `_cropConfirm` inverse-maps the gold-circle bounds to image coordinates, draws into a 512×512 canvas via `drawImage(img, srcX, srcY, srcSize, srcSize, 0, 0, 512, 512)`, exports `image/jpeg` q0.92 via `toBlob`, then hands the cropped File to the unchanged `handleAvatarUpload` pipeline. `closeAvatarCropModal` does NOT touch `AppState.ui.modalOpen` or `document.body.style.overflow` — the parent profile panel still owns both. `handleAvatarUpload(file)` validates type/size (<5MB), compresses via `_compressAvatar` (max 400×400 JPEG q0.80), uploads to R2 `profile-pictures/{sanitizedEmail}.jpeg` (sanitize: @ → -at-, . → -), then builds `displayUrl = 'https://images.srtd.io/' + key + '?t=' + Date.now()` and PATCHes that **cache-busted URL** into `profiles.avatar_url` so refreshes also see the new image (storing the clean URL was the stale-avatar-after-refresh bug). The same `displayUrl` is mirrored into `_profilesCache[email].avatar_url` and `AppState.user.avatarUrl` so the current session reflects the change immediately. The R2 upload worker sets `Cache-Control: public, max-age=31536000, immutable` on every `profile-pictures/` object — safe because each upload's `?t=` query string makes the URL unique. **Avatar render path is no-strip**: `getAvatarUrl()` returns `cache.avatar_url` verbatim and `renderAvatar()` only HTML-escapes the double-quote before injecting into `<img src>` — neither helper splits or replaces the `?` query separator. All 13 `renderAvatar()` call sites (03-auth.js, 07-post-load.js, 10-ui.js ×2, 12-profiles.js ×2, actions/pcs.js ×2, render/brief.js ×2, render/dashboard.js, render/client.js ×2) inherit this contract; do not introduce a strip helper anywhere upstream of `<img src>`. Scratchpad: profile panel includes a textarea that auto-saves on blur to the `scratchpad` column; also included in `saveProfile()` PATCH payload.
