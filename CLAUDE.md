...
Last updated: 2026-04-16 (fix-pcs-caption-save: five fixes for `_saveCaptionEdit` reliability (actions/pcs.js:2812). (1) Added `_isSaving` guard + flag — `post._isSaving = true` before PATCH, cleared in both success and catch paths. Activates the `mergePosts` skip guard (07-post-load.js:125) so Realtime/poll refreshes cannot overwrite optimistic caption state during an in-flight save. (2) Replaced reference-swap `Object.assign({}, p, ...)` + `setAll()` with in-place `Object.assign(post, { caption, updated_at })` — keeps `AppState.pcs.post` reference intact per §4 Mutation Rules, eliminating the caption-revert-on-reopen bug. (3) Added `updated_at: new Date().toISOString()` to the PATCH body so the posts row timestamp advances on caption edits (previously only sent `caption` + `updated_by`). (4) Added `showToast('Caption saved', 'success')` on successful save — previously zero visual feedback on success. (5) Added `_startSaveTimeout(post, postId)` before PATCH and `_clearSaveTimeout(post)` in both success/catch paths — wires the 10-second safety net from 08-post-actions.js so a hung PATCH cannot permanently lock the post. Root cause of the "click save many times, nothing happens, caption reverts" reports from April 14-15: the old function had 2/10 safety patterns (guardAction + error toast) but was missing _isSaving, in-place mutation, updated_at, success toast, and save timeout — the 8 patterns that quickStage implements correctly. Zero schema changes.) Full history: `CLAUDE-archive-20260411.md`.
...
1. Bump ALL 23 `?v=YYYYMMDDx` strings in `index.html` together (1 stylesheet + 22 scripts). Current: `?v=20260416b`. The Supabase JS SDK `<script>` tag sits ABOVE the versioned block and is pinned to an external jsDelivr URL — do NOT add a `?v=` to it.
