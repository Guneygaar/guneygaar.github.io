...
Last updated: 2026-04-15 (audit-fix-duplicates-auth: four hardening fixes for Client-role 401 eviction + duplicate REST traffic. (1) `loadPostsForClient` now ALWAYS threads `{ allowLogout: false }` through its apiFetch calls (07-post-load.js:282). Previously, only background polls (`fromPoll=true`) got the flag; first-render and user-initiated loads defaulted to `allowLogout:true`. Combined with the parallel fan-out in `activateRole()` client branch (loadPostsForClient + fetchProfiles + updateLastActive, no await), a transient 401 on ANY chain would call `_clearSessionAndLogin()` and cascade-evict the other parallel chains. Root cause of the 187 "Supabase 401: session expired" error_log rows seen April 5-15, all Client role. (2) `fetchProfiles` and `updateLastActive` (12-profiles.js:13, 137) and `_fetchClientMentionRoster` (render/client.js:1552) now thread `{ allowLogout: false }` for the same parallel-fan-out reason. After these fixes, ONLY the visibilitychange handler (03-auth.js:162) and the 50-min proactive refresh timer (03-auth.js:372) can call `_clearSessionAndLogin()`. Every apiFetch caller reachable from activateRole is now shielded. (3) Cross-tab refresh lock race closed (03-auth.js refreshSession/_doRefresh): the `localStorage.setItem('_srtd_refresh_lock', ...)` call moved OUT of `_doRefresh` (which sat after a `fetch` await) and INTO `refreshSession`, in the SAME synchronous block as the `parseInt(localStorage.getItem('_srtd_refresh_lock') ...)` read. Lock removal moved to the refreshSession finally block. Fixes a narrow window where two tabs could both read lockTs as empty, both call _doRefresh with the same refresh_token, and trigger Supabase's refresh-token-reuse revocation. (4) Duplicate `fetchProfiles()` call removed from 04-router.js (`_startRouter` was calling it AFTER `activateRole()`, but activateRole already calls it internally on every branch). Added the missing `fetchProfiles()` + `updateLastActive()` calls to the admin-preview early-return branch of activateRole (03-auth.js:398-414) so the preview path still warms the profiles cache. Eliminates 2× GET /profiles + 2× GET /user_roles on every session resume. Zero schema changes.) Full history: `CLAUDE-archive-20260411.md`.
