...
Last updated: 2026-04-26 (PR-3.17 — History tab replaces Activity in PCS. New table `post_versions` (PK id uuid, post_id uuid FK posts.id ON DELETE CASCADE, caption text, edited_by text, edited_by_role text, edited_at timestamptz default now()) captures caption edits via BEFORE UPDATE trigger `posts_capture_caption_version` on `posts` (WHEN OLD.caption IS DISTINCT FROM NEW.caption) calling SECURITY DEFINER fn `capture_post_version()` which COALESCEs editor email from NEW.updated_by / OLD.updated_by / `current_setting('request.jwt.claims',true)::json ->> 'email'` / 'system' and looks up role from `user_roles`. Index `post_versions_post_id_edited_at_idx (post_id, edited_at DESC)`. RLS enabled, single SELECT policy `post_versions read for authenticated` USING true. Schema applied to live Supabase 2026-04-26 via Supabase MCP — `sql/013-post-versions.sql` is parity-only documentation + replay reference (slot 011 was taken by `011-plan-foundation.sql` and 012 by `012-plan-spawn-rls-rpcs.sql`, so PR-3.17 lands at the next free slot 013). Frontend: new `srtd-next/src/core/api/history.js` exports `listHistoryForPost(postId, userRolesCache)` which fires two parallel `apiFetch` reads (post_versions ordered `edited_at.desc`, activity_log ordered `created_at.desc`), normalizes both into `{id, type, actor, actorRole, at, caption?|oldStage?+newStage?, action?}`, derives `type` as `caption` for post_versions rows, `stage` when `new_stage` is set or action matches `/stage|approved|moved/i`, otherwise `system`; resolves actorRole via `user_roles` cache lookup with `ownerToRole` normalisation, fallback `unknown`. Type mismatch flagged: `post_versions.post_id` is uuid (passes `post.id`) while `activity_log.post_id` is text — caller passes a single `postId`; mismatch deliberately NOT addressed in this PR. New `srtd-next/src/flows/pcs/components/HistoryFeed.jsx` mounts via `useEffect` on `post.id` + `userRoles`, render-side collapses consecutive same-author `caption` rows within a 15-minute window into a single run that displays `Edited caption {N} times` + relative-time range and offers a `Show diff` toggle that mounts inline `diffWords(mergedFromCaption, latestToCaption)` from `diff` (jsdiff) — added (`text-green` underlined), removed (`text-text-dim` line-through), unchanged plain; whitespace-pre-wrap container preserves newlines. Stage rows render `prettyStage(oldStage) → prettyStage(newStage)` via existing `STAGE_LABELS`. System rows are suppressed when their action matches `/caption/i` AND a caption row already exists in the feed (avoids double-counting). Toggle state is row-local `useState`, NOT pcsStore. Dot component: 8px circle, terracotta for caption rows, text-dim otherwise — both via Tailwind tokens, dual-mode auto-flips. `Tabs.jsx` third-tab label `Activity` → `History`, internal value `activity` → `history`; tab order Client `Comments | History`, Agency `Comments | Internal | History`. `PCS.jsx` drops `ActivityFeed` import, adds `HistoryFeed`, swaps the body branch (`activeTab === 'history' ? <HistoryFeed post={post} userRoles={userRoles} /> : null`), updates Composer gate `activeTab !== 'history'`. `srtd-next/src/flows/pcs/components/ActivityFeed.jsx` deleted. New dep `diff@^9` (~6-8 kB gzip). Bundle rebuilt: `dist/sorted-react.js` 632.20 kB / `dist/style.css` 38.22 kB. `window.SortedReact.version` bumped `0.9.33 -> 0.9.34`. 28 `?v=` strings bumped `20260426f -> 20260426g`. NO Worker changes. Prior entry: PR-3 — Plan spawn, notification suppression, share token, RLS (`sql/012-plan-spawn-rls-rpcs.sql`, edge fns `align-plan` + `send-plan-alignment`, RPCs `align_plan_transaction` + `send_plan_for_alignment_transaction` + `generate_plan_share_token`, DB BEFORE INSERT trigger `suppress_plan_spawned_stage_notifs` on `notifications`, RLS on plans / plan_cells / plan_versions / plan_comments with anon `share_token` access, frontend wiring in `srtd-next/src/flows/plan/api/planTablesApi.js` + `planStore.js sendPlanForAlignment` / `alignPlan` / `sharePlan`). Earlier PR-2 (PlanView UI: `PlanView.jsx`, `BottomNav` 5-tab, `realtimeBridge.js startPlanCellsBridge`, `PlanSheets.jsx` slide-up sheets), PR-1 (`sql/011-plan-foundation.sql` plans / plan_cells / plan_versions / plan_comments + `posts.plan_cell_id`), and PR-3.15 series (PostDeleteConfirm, e2e React selectors, `posts.internal_notes` DROP, `#pcs-react-overlay` id) archived in full to `CLAUDE-archive-20260411.md`.
## 1 — WHAT IS SORTED
