...
Last updated: 2026-04-16 (fix-caption-edit-ui: four UI wiring fixes for caption edit mode (actions/pcs.js `_startCaptionEdit`/`_cancelCaptionEdit`/`_saveCaptionEdit`). (1) Duplicate textarea guard — `_startCaptionEdit` now returns immediately if `#pcs-caption-textarea` already exists, preventing stacked edit UIs from repeated Edit taps. (2) Hide `.pcs-zone-manual` footer (Edit/Copy/Clear/Done) during edit mode — the old code targeted nonexistent `#pcs-caption-edit-btn` so the Edit button stayed clickable; now hides the entire footer via `querySelector('#pcs-pane-caption .pcs-zone-manual').style.display='none'`, restored in both `_cancelCaptionEdit` and `_saveCaptionEdit`. (3) Hide `#pcs-caption-see-more` during edit mode — the See More/Less toggle sat between the textarea and Save button; now hidden on edit entry and restored on save/cancel. (4) Enlarged SAVE/CANCEL tap targets — `padding:6px 12px` → `padding:14px 20px` + `min-height:44px` to meet Apple's 44px minimum tap target guideline, removed redundant `margin-left:6px` on CANCEL (btnRow `gap:8px` handles spacing). Also removed dead `editBtn` (`#pcs-caption-edit-btn`) references from all three functions. Zero schema changes.) Full history: `CLAUDE-archive-20260411.md`.
...
1. Bump ALL 27 `?v=YYYYMMDDx` strings in `index.html` together (1 stylesheet + 26 scripts). Current: `?v=20260418m`. The Supabase JS SDK `<script>` tag sits ABOVE the versioned block and is pinned to an external jsDelivr URL — do NOT add a `?v=` to it.
