...
Last updated: 2026-04-12 (notif-thread-polish2). Full history: `CLAUDE-archive-20260411.md`.
...
- Meta row (`.notif-meta`): flex with `gap: 10px`. Layout is `time [chip] <.meta-actions>WA trash</.meta-actions> [LinkedIn]`. `.meta-actions` wraps the WA + trash icon buttons with an internal `gap: 2px` and `margin-left: auto`, so the icon cluster is always pushed to the right edge of the row. No `.notif-meta-sep` dot spans are emitted anywhere — the class is kept as a no-op `display:none` so stale HTML never breaks layout. `.notif-mi-btn` is a 28×28 rounded-6px tap target with `background: #FFFFFF0A` on hover. SVG children are `pointer-events:none`. Panel tap delegate skip list includes `.notif-mi-btn`, `.notif-topbar-btn`, `.notif-li-link`, `.meta-actions`, and every thread-view control below.
...
- Drawer visuals: `.thread-area` is `#1C1926` + `border: 1px solid #C8A84B1F` + `border-radius: 10px` + `padding: 10px 12px 0` so the drawer is clearly distinct from the panel. `.notif-item.expanded` uses `background: #16161F` + `border-left: 3px solid #C8A84B4D` + `border-radius: 10px` + `padding-bottom: 0` + `margin-bottom: 8px`. `.notif-item.expanded .thread-drawer` uses `max-height: 3000px` so the reply row + "Open full post →" link never clip. `.thread-reply` carries `border-top: 1px solid #C8A84B1F` + `padding-top: 8px` + `padding-bottom: 10px`. Thread message typography: `.thread-av` 22×22 (vs 32px on cards), `.thread-author` 11px, `.thread-role` 8px pill on `#FFFFFF12`, `.thread-time` 9px `#FFFFFF38`, `.thread-message` 12px `#FFFFFFB3`.
