...
Last updated: 2026-04-15 (pcs-topbar-admin-delete: PCS topbar right zone now renders an admin-only red trash icon (`.pc-icon-btn.danger`, 15×15 SVG) BEFORE the existing WhatsApp icon. The delete icon is gated on `_pcsRole === 'admin'` and calls `window.pcsConfirmDelete()` which routes to the pre-existing `pcsDoDelete()` confirm-sheet flow at `actions/pcs.js:2120-2165`. This wires up the previously-orphaned PCS delete path — `deletePost()` + `#ae-delete-btn` in the Admin Edit modal are still orphaned but no longer the only delete entry point. Zero CSS / HTML / schema changes — reuses `.pc-icon-btn.danger` (styles.css:3151-3166) and the existing `#pcs-topbar-right` flex container (`gap:6px`, `align-items:center`) inline-styled at index.html:950. Change site: `actions/pcs.js:801-810`.) Also 2026-04-15 (claude-md-ai-sync, doc-only): backfilled the AI-foundation subsystem that was already merged but not yet in CLAUDE.md — `srtd-ai-worker/` Cloudflare Worker (§6), `ai-config.js` root file + `window.AI_CONFIG` contract (§3), `workspace_settings` + `ai_usage` tables (§2), `posts.is_draft` + `posts.ai_origin` columns (§2), `loadWorkspaceSettings()` helper in `05-api.js` (§3), Section 3 header file count fix (22 → 23 versioned resources, 21 → 22 js). Zero code changes. Full history: `CLAUDE-archive-20260411.md`.
...
Repo: `github.com/Guneygaar/guneygaar.github.io`. ALL files at REPO ROOT. Never reference `/sorted/` — it does not exist. Subdirs: `render/ actions/ tests/ tests/e2e/ sorted-preview-worker/ srtd-ai-worker/ sql/ preview/ mockups/`.
...
| images | jsonb | |
| is_draft | bool DEFAULT false | true = AI-created draft, admin-only visibility. Every non-Admin query in 07-post-load.js scopes with `is_draft=eq.false`. |
| ai_origin | bool DEFAULT false | permanent provenance flag, never flipped back. Drives the ✦ marker in AI features. |

NO `comments` column on posts. Never write to it. Migration files: `sql/003-posts-ai-columns.sql`.
...
**workspace_settings** — PK workspace_id(text, single-tenant `'default'`). Feature flags consumed by the `srtd-ai` Worker and the frontend. AI-relevant cols: `ai_enabled`, `ai_writer`, `ai_qc`, `ai_chat`, `ai_email_briefs` (all bool). Fetched once at login by `loadWorkspaceSettings()` (05-api.js) and stashed on `window.AppState.workspace` so every AI feature gate can read it without a network call. Fire-and-forget, never blocks login, swallows errors into `{}` so the app degrades gracefully.

**ai_usage** — PK id(uuid). Per-call telemetry sink for every Anthropic API invocation made by the `srtd-ai` Worker. Cols: workspace_id(text DEFAULT 'default'), post_id(text, NO FK because email-brief flows log before the post exists), feature(text: `writer|qc|chat|email_brief`), tokens_input(int), tokens_output(int), cost_usd(numeric(10,6)), created_by(text), created_at(timestamptz). RLS DISABLED — Worker writes with service-role key, frontend never reads this table directly. Cost formula (Claude Sonnet 4): `((tokens_input * 3) + (tokens_output * 15)) / 1e6`. Migration: `sql/004-ai-usage-table.sql`.

## 3 — FILE MAP

23 versioned resources (1 css + 22 js). `00-appstate.js` + `00-appstate-compat.js` have NO `defer`. `04-router.js` MUST be the LAST script. `actions/pcs-longpress.js` must load AFTER `actions/pcs.js`. `12-profiles.js` must load AFTER `utils.js` and BEFORE `03-auth.js`. `ai-config.js` must load BEFORE any file that reads `window.AI_CONFIG`.
...
- `05-api.js` — apiFetch (no-cache headers, 401 retry), uploadPostAsset, logActivity, normalise, `loadWorkspaceSettings()` (one-shot fetch of `/workspace_settings?workspace_id=eq.default` at login, stashed on `window.AppState.workspace`, fire-and-forget, `allowLogout:false`, swallows errors into `{}`)
- `ai-config.js` — single source of truth for the `srtd-ai` Cloudflare Worker URL, shared secret, and default Claude model. Every AI feature reads from `window.AI_CONFIG = { workerUrl, secret, model }`. NEVER hardcode the Worker URL elsewhere.
...
- **srtd-ai** — `https://srtd-ai.ksg-kumarshubhamgune.workers.dev` (URL also in `window.AI_CONFIG.workerUrl`). Source: `srtd-ai-worker/src/index.js`. Single Worker that fronts the Anthropic API for every AI feature. Gates each request per-workspace via `workspace_settings.ai_*` flags (`ai_enabled`, `ai_writer`, `ai_qc`, `ai_chat`, `ai_email_briefs`) before calling Claude, then logs every call to `ai_usage` fire-and-forget. Secrets: `ANTHROPIC_API_KEY`, `AI_SECRET=srtd-ai-2026xK9mN3pQ` (must match `window.AI_CONFIG.secret`). R2 binding `AI_ASSETS` → bucket `sorted-ai`, optional key `ai/brand-guide.txt`. Endpoints: `POST /ai/complete` (full completion + log), `POST /ai/log` (log-only for flows that don't round-trip through the Worker). Deploy: `cd srtd-ai-worker && wrangler deploy`. See `srtd-ai-worker/README.md` for first-time setup.
