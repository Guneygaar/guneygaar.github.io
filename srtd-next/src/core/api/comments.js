...
export async function listComments(postId) {
  ...
  const rows = await apiFetch(
    `/post_comments?post_id=eq.${encoded}&deleted=not.eq.true&visibility=eq.all&order=created_at.desc&select=*`,
...
export async function listInternalNotes(postId) {
  ...
  const encoded = encodeURIComponent(postId);
  const rows = await apiFetch(
    `/internal_notes?post_id=eq.${encoded}&deleted=not.eq.true&order=created_at.desc&select=*`,
    { method: 'GET', headers: { 'Accept': 'application/json' } }
  );
  return Array.isArray(rows) ? rows : [];
}
