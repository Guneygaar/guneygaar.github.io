...
export function CommentList({ comments, reactions, userRoles, currentEmail, emptyLabel = 'No comments yet' }) {
  ...
  return (
    <div>
      {visible.map(c => (
        <CommentRow key={c.id} comment={c} byId={byId} userRoles={userRoles} reactions={reactions} currentEmail={currentEmail} />
      ))}
    </div>
  );
}
