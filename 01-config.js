...
/* Improvement 1: generate DB stages automatically */
const STAGES_DB = Object.keys(STAGE_META).filter(s => s !== 'archive');
...
/* Improvement 2: safe STAGE_DISPLAY builder */
const STAGE_DISPLAY = {};
Object.keys(STAGE_META).forEach(k => {
  STAGE_DISPLAY[k] = STAGE_META[k].label;
});
...
const CREATIVE_URGENCY = [
  'revisions needed',
  'awaiting brand input',
  'in production'
];

const NEXT_POST_URGENCY = [
  'revisions needed',
  'awaiting brand input',
  'in production',
  'ready',
